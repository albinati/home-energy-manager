...
| `LP_SOLAR_CHARGE_FOX_MODE` | `backup_fill` | Fox mode for `solar_charge` holds (#679). `backup_fill` (default) = Backup(minSoc=reserve, maxSoc=LP-target) — strict no-discharge hold that lets PV fill toward target WITHOUT grid-charging (grid-charge in Backup is minSoc-driven); fill curve verified in prod via #685, instant rollback if grid-import appears. `backup_hold` = Backup(reserve,reserve) — pure hold that blocks the PV fill (exports surplus); same tuple as A1 pre-peak holds. `selfuse` = plain SelfUse at reserve (does NOT hold). The retired SelfUse(100,100) shape is never emittable (A0). |
