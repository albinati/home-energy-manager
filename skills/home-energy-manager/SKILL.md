...
- Tailscale hostname: `<hem-host>.your-tailnet.ts.net` (IP: 100.x.y.z)
- SSH (no keys needed — Tailscale SSH): `ssh root@<hem-host>.your-tailnet.ts.net`
...
**Local machine (WSL):**
- Tailscale hostname: `<workstation>.your-tailnet.ts.net` (IP: 100.x.y.z)
- Windows host: `<workstation>-1.your-tailnet.ts.net` (IP: 100.70.167.7)
...
./scripts/deploy_hetzner.sh --backup  # backup DB first (needs SSH on <workstation>)
```

**Deploy (from agent/local — SSH through Tailscale):**
```bash
ssh root@<hem-host>.your-tailnet.ts.net "cd /root/home-energy-manager && ./scripts/deploy_hetzner.sh"
```
...
```bash
ssh root@<hem-host>.your-tailnet.ts.net
cd /root/home-energy-manager
```
