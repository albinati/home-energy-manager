            ...
            for row in rows:
                conn.execute(
                    """INSERT OR REPLACE INTO forecast_skill_log
                       (date_utc, hour_of_day, predicted_temp_c, actual_temp_c,
                        predicted_pv_kwh, actual_pv_kwh, predicted_load_kwh,
                        actual_load_kwh, built_at_utc)
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                    (
                        str(row["date_utc"]),
                        int(row["hour_of_day"]),
...
def rebuild_forecast_skill_log_for_date(date_utc: str) -> int:
    ...
    Prediction source:
    the most recent canonical forecast fetch strictly before each
    ...
    with _lock:
        ...
        try:
            cur = conn.execute(
                """SELECT mv.forecast_fetch_at_utc, mv.slot_time, mv.temp_c, mv.solar_w_m2, mv.cloud_cover_pct
                   FROM meteo_forecast_value mv
                   JOIN meteo_forecast_snapshot ms
                     ON ms.forecast_fetch_at_utc = mv.forecast_fetch_at_utc
