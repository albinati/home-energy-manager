...
Two middlewares live here, and both are mounted in :mod:`src.api.main`:
...
* :class:`ApiV1RoleAuth` — gates ``/api/v1/*`` by ROLE rather than by a
  single token: viewer-open reads, admin-gated writes, plus a scoped
  sensor-ingest token that satisfies exactly one write route. Gated by
  ``HEM_UI_AUTH_REQUIRED``.

(A third, ``ApiV1BearerAuth``, was removed in 2026-07: it was never
mounted, so the module previously documented a guard that did not run.)
