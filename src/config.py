...
class Config:
    ...
    # A NON-admin write credential that unlocks ONLY POST /api/v1/sensors/indoor
    # (see ApiV1RoleAuth.ingest_tokens). This is what an internet-exposed device
    # (an ESPHome room sensor, pushing through the locked-down `hem-ingest`
    # proxy + Tailscale funnel) carries — so a firmware/network leak can only
    # post fake temperatures to that one route, never touch admin. Empty →
    # feature off (only admin tokens work, as before). Rotate to revoke a device.
